...
function loadClassManagement() {
    console.log('加载班级管理页面...');
    
    // 显示加载状态
    const originalContent = document.getElementById('contentArea').innerHTML;
    document.getElementById('contentArea').innerHTML = `
        <div class="d-flex justify-content-center align-items-center" style="height: 300px;">
            <div class="spinner-border text-primary" role="status">
                <span class="visually-hidden">加载中...</span>
            </div>
            <span class="ms-3">正在加载班级管理...</span>
        </div>
    `;
    
    fetch('/class/management/dashboard')
        .then(response => {
            if (!response.ok) {
        ...
        .then(html => {
            ...
            // 移除可能存在的header部分
            const headerElements = tempDiv.querySelectorAll('header, .navbar, .navbar-custom');
        ...
        .catch(error => {
            ...
            document.getElementById('contentArea').innerHTML = 
                '<div class="alert alert-danger">加载班级管理页面失败，请刷新重试</div>';
        });
}*/
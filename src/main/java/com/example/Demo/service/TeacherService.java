...
public class TeacherService {
    ...
    public List<Map<String, Object>> getTeacherList() {
        String sql = "SELECT t.*, d.department_name, " +
                    "(SELECT COUNT(*) FROM course c WHERE c.teacher_id = t.id) as course_count " +
    ...
    public Map<String, Object> getTeacherById(Long id) {
        String sql = "SELECT t.*, d.department_name FROM teacher t " +
                    "LEFT JOIN department d ON t.department_id = d.id WHERE t.id = ?";
    ...
    public boolean addTeacher(Map<String, Object> teacherData) {
        ...
        try {
            jdbcTemplate.update(sql, 
                teacherData.get("teacher_id"), 
                teacherData.get("name"),
                teacherData.get("gender"), 
                teacherData.get("department_id"), 
                teacherData.get("title"),
                teacherData.get("phone"), 
                teacherData.get("email"), 
                teacherData.get("office")
            );
            return true;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean updateTeacher(Long id, Map<String, Object> teacherData) {
        String sql = "UPDATE teacher SET teacher_id = ?, name = ?, gender = ?, " +
                    "department_id = ?, title = ?, phone = ?, email = ?, office = ? " +
                    "WHERE id = ?";

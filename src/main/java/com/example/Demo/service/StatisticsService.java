...
public class StatisticsService {
	...
	public Map<String, Integer> getScoreDistribution() {
		String sql = "SELECT " + "SUM(CASE WHEN score >= 90 THEN 1 ELSE 0 END) as excellent, "
				...
				+ "SUM(CASE WHEN score < 60 THEN 1 ELSE 0 END) as fail " + "FROM grade WHERE exam_type = '期末'";
	...
	public Map<String, Integer> getCoursePopularity() {
		String sql = "SELECT c.course_name, COUNT(g.course_id) as student_count "
				+ "FROM course c LEFT JOIN grade g ON c.course_id = g.course_id "
				+ "GROUP BY c.course_id, c.course_name ORDER BY student_count DESC LIMIT 10";
	...
	public List<Map<String, Object>> getStudentGrowthData() {
		String sql = "SELECT enrollment_year, COUNT(*) as count " + "FROM student WHERE is_active = TRUE "
				+ "GROUP BY enrollment_year ORDER BY enrollment_year";
		return jdbcTemplate.queryForList(sql);
	}

	public List<Map<String, Object>> getGradeTrendData() {
		String sql = "SELECT exam_date, AVG(score) as avg_score " + "FROM grade WHERE exam_type = '期末' "
	...
	public boolean generateStatisticsReport(Map<String, Object> reportConfig) {
		...
		try {
			jdbcTemplate.update(sql, reportConfig.get("report_type"), reportConfig.get("report_name"),
					reportConfig.get("generated_by"), reportConfig.get("data_json"));
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
